...
动态指导助手让作者把完整剧情流程、物品出现时机、地点规则或秘密写在一个角色绑定世界书条目中。脚本会禁用这个来源条目，并在每次生成前只向 AI 注入当前有效内容。过去与未来的内容不会进入本次提示词。进度只记在当前聊天里，新建聊天从第一段开始。
...
10. 保存或导出角色卡。阶段划分同时写进世界书条目的隐藏扩展数据和条目正文末尾的隐藏标记，会跟随条目一起分享；别人导入角色卡后直接绑定就能玩。

已经有旧版模板的用户可以继续沿用 `【内容：名称】` 格式，不需要重新划分。

## 进度只记在当前聊天里

绑定关系和阶段划分记在角色身上，剧情进度只记在当前聊天里。

- 每个聊天各自记着自己的进度，改动一个聊天不会影响其他已经进行中的聊天。
- 新建聊天从第一段开始，不会继承其他聊天推进到的位置。
- “重置到第一段”只影响当前聊天。
- 重新绑定指导条目时，当前聊天也会回到第一段。
...
```js
import 'https://gcore.jsdelivr.net/gh/PlayerAlex/sillytavern-dynamic-guide@v1.3.6/index.js';
```
...
每个被标记的文字范围会记录“原文片段、前面一段文字、后面一段文字”三个定位信息。整份划分同时保存在三处：

1. 条目的隐藏扩展数据 `entry.extra.dynamicGuideAssistant.layout`，本机读取时优先使用。
2. 条目正文末尾的一段隐藏标记 `<!-- DGA_LAYOUT_V1:BEGIN --> … <!-- DGA_LAYOUT_V1:END -->`。这段标记跟着条目、跟着角色卡导出，别人导入后即使本机没有扩展数据也能直接读出划分。脚本在标注器、管理页预览和发送给 AI 的内容里都会把它去掉，作者只会在世界书编辑器里看到它。手动删掉这段标记不会损坏条目，只是划分退回到只靠扩展数据和备份。
3. 角色变量和全局变量里的备份 `$dynamicGuideAssistant.layouts`。世界书条目里两处都丢失时，脚本会从备份恢复并自动写回。

有了定位信息，作者之后再修改大纲、在段落前面插入新文字或改写周边描述时，脚本会自动把标记挪回正确的位置。如果某段被标记的文字被整段删除，管理页会提示“有选区无法定位”，此时打开标注器重新选择那一段即可。
...
- 可视化阶段划分保存在世界书条目的隐藏扩展数据 `entry.extra.dynamicGuideAssistant.layout` 和正文末尾的隐藏标记里，并在角色变量、全局变量 `$dynamicGuideAssistant.layouts` 中留一份备份。
- 当前聊天的进度保存在聊天变量 `$dynamicGuideAssistant.state`，新聊天没有这份变量时从第一段开始。
...
- 新聊天从第一段开始；如果想在新聊天里直接跳到后面的阶段，在管理页点“下一段”即可。
