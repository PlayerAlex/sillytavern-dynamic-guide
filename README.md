...
动态指导助手让作者把完整剧情流程、物品出现时机、地点规则或秘密写在一个角色绑定世界书条目中。脚本会禁用这个来源条目，并在每次生成前只向 AI 注入当前有效内容。过去与未来的内容不会进入本次提示词。
...
7. 选中属于这一阶段的文字。手机默认用“点选头尾”：点一下开头、再点一下结尾，中间会自动选中；电脑或习惯拖选的用户可以在“选择方式”里切回“拖选文字”。选好以后点击“分配给当前阶段”。同一阶段可以选多处不连续文字。
...
```js
import 'https://gcore.jsdelivr.net/gh/PlayerAlex/sillytavern-dynamic-guide@v1.3.2/index.js';
```
...
- 游玩进度保存在聊天变量 `$dynamicGuideAssistant.state`，每个聊天互不影响。
